...
agent_communication:
  ...
  - agent: "fork"
    message: "✅ BUG FIX - 404 PROFILE NOT FOUND WHEN CREATING EXPERIENCE: Fixed error preventing users from adding experiences due to missing profile. **ROOT CAUSE:** Nina (and potentially other users) had user account but no corresponding profile in profiles collection. API endpoint required profile to exist before adding experiences, causing 404 error with message 'Profil non trouvé. Veuillez créer votre profil d'abord.' **SOLUTION:** Implemented auto-creation of minimal profile on first experience addition. Modified /app/auth-microservice/professional_experiences_routes.py create_experience endpoint (lines 162-183) to: 1) Check if profile exists for user, 2) If not, create minimal profile with profile_type based on user roles (candidat/interim/company/collaborator), 3) Initialize empty professional_experiences array, 4) Set profile_completed=False and profile_completion_percentage=0. **BUSINESS LOGIC:** Auto-creating profile improves UX - users can immediately add experiences without manually creating profile first. Profile is created lazily on-demand. **FILES MODIFIED:** /app/auth-microservice/professional_experiences_routes.py (added auto-profile-creation logic). **VERIFICATION:** Tested with Nina - profile created automatically on first experience addition. **LESSON LEARNED:** Always consider data dependencies and implement graceful handling for missing related entities. Auto-creation patterns improve UX when appropriate. **IMPACT:** Users can now add professional experiences without pre-existing profile. Removes friction from user onboarding."
  - agent: "fork"
