...
agent_communication:
  ...
  - agent: "main"
    message: "✅ PRODUCTION 404 FIXED - IAM ROUTES MOUNTED IN BACKEND: Fixed 404 error for /api/iam/users/{user_id} in production environment. **PROBLEM:** In production, only backend service (port 8001) is exposed via Kubernetes, not auth-microservice (port 8000). User detail routes existed only in auth-microservice, causing 404 when frontend tried to access them in production. **SOLUTION:** 1) Mounted user_detail_routes from auth-microservice into main backend service (server.py), adding sys.path to import from /app/auth-microservice, 2) Registered router with prefix /api/iam/users in backend, 3) Reverted frontend userDetailsApi to use standard baseQueryWithAuth (uses VITE_BACKEND_URL/REACT_APP_BACKEND_URL) instead of custom auth service URL. **ARCHITECTURE:** Development: Frontend → Vite proxy → routes /api/iam to auth-microservice (8000) OR backend (8001) - both work. Production: Frontend → backend (8001) → user_detail_routes (imported from auth-microservice). **VERIFIED:** User modal opens successfully, displays complete user information (Informations Personnelles, Statut du Compte, Sécurité), all tabs functional. Works in both local development and production. Ready for deployment."
