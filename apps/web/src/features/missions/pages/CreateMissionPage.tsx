...
export default function CreateMissionPage() {
  ...
  const [formData, setFormData] = useState<Partial<MissionCreate>>({
    title: '',
    description: '',
    company_id: user?.id || '',
    job_type: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.title || !formData.description || !formData.job_type || !formData.location) {
      toast.error('Veuillez remplir tous les champs obligatoires')
      return
    }

    try {
      const result = await createMission(formData as MissionCreate).unwrap()
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-jlc-purple-500"
                  placeholder="Ex: 2-5 ans, Débutant accepté..."
                  required
                />
              </div>
