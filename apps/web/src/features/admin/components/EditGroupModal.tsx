...
export default function EditGroupModal({ group, isOpen, onClose }: EditGroupModalProps) {
  ...
  const { data: usersData } = useGetUsersQuery({ page: 1, page_size: 1000 })
      ...
      <div className="flex min-h-screen items-center justify-center p-4">
        ...
        <div className="relative bg-white rounded-lg shadow-xl max-w-2xl w-full p-6 z-10 max-h-[90vh] overflow-y-auto">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xl font-semibold text-gray-900">Modifier le groupe</h3>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Profil de permissions</label>
              <select
                ...
                {usersData?.users && usersData.users.length > 0 ? (
                  <div className="divide-y divide-gray-200">
                    {usersData.users.map((user) => (
                      <label key={user.id} className="flex items-center px-4 py-2 hover:bg-gray-50 cursor-pointer">
                        <input
                          type="checkbox"
