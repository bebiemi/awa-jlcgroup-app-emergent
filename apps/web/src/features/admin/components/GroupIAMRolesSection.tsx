...
interface GroupIAMRolesSectionProps {
  groupId: string
  groupName: string
}

export default function GroupIAMRolesSection({ groupId, groupName }: GroupIAMRolesSectionProps) {
  const { data: groupRolesData, isLoading: loadingGroupRoles } = useGetGroupRolesQuery(groupId)
  const { data: allIAMRoles = [], isLoading: loadingAllRoles } = useListIAMRolesQuery()
  
  const [assignRoles] = useAssignRolesToGroupMutation()
  const [removeRole] = useRemoveRoleFromGroupMutation()
  
  const [selectedRoleId, setSelectedRoleId] = useState('')

  const groupRoles = groupRolesData?.iam_roles || []
  ...
  const handleAssignRole = async () => {
    ...
    try {
      ...
      toast.success('Rôle IAM assigné au groupe avec succès')
      setSelectedRoleId('')
    } catch (error: any) {
  ...
  const handleRemoveRole = async (roleId: string, roleName: string) => {
    if (!confirm(`Êtes-vous sûr de vouloir retirer le rôle "${roleName}" de ce groupe ?`)) {
      return
    }
    
    try {
      await removeRole({ groupId, roleId }).unwrap()
      toast.success('Rôle IAM retiré du groupe avec succès')
    } catch (error: any) {
  ...
  const availableRoles = allIAMRoles.filter(
    (role: any) => !groupRoles.some((gr: any) => gr.id === role.id)
  )
    ...
    <div className="bg-white rounded-lg shadow p-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Rôles IAM assignés au groupe <span className="font-medium">{groupName}</span>
          </p>
        </div>
      ...
      {loadingGroupRoles ? (
        ...
        <div className="text-center py-8 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
          <p className="text-gray-500 text-sm">Aucun rôle IAM assigné à ce groupe</p>
        ...
        <div className="space-y-2">
          {groupRoles.map((role: any) => (
            <div 
              key={role.id} 
              className="flex items-center justify-between p-4 bg-gradient-to-r from-indigo-50 to-white border border-indigo-200 rounded-lg hover:shadow-md transition-shadow"
            >
      ...
      {groupRoles.length > 0 && (
        <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
          <p className="text-xs text-blue-800">
            <strong>Note:</strong> Les membres de ce groupe hériteront automatiquement 
