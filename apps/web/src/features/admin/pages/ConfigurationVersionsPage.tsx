        ...
        >
          <div className="space-y-4">
            <div>
              ...
              />
              <p className="mt-1 text-xs text-gray-500">{selectedVersion?.description}</p>
            </div>
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setVersionToCompare(e.target.value)}
              ...
              <div className="space-y-4">
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <h4 className="text-sm font-medium text-blue-900 mb-2">
                    Comparaison : {compareResult.version_from} → {compareResult.version_to}
                  </h4>
                  <div className="grid grid-cols-3 gap-4 text-sm">
                    <div className="text-center">
                      ...
                      <div className="text-gray-600">Ajoutés</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-orange-600">
                        {compareResult.modified?.length || 0}
                      </div>
                      <div className="text-gray-600">Modifiés</div>
                    </div>
                    <div className="text-center">
                ...
                {compareResult.added && compareResult.added.length > 0 && (
                  <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                    <h5 className="text-sm font-medium text-green-900 mb-2 flex items-center gap-2">
                      ...
                      Ajoutés ({compareResult.added.length})
                    </h5>
                    <ul className="space-y-1">
                      {compareResult.added.map((item: string, idx: number) => (
                        <li key={idx} className="text-xs text-green-800 font-mono">
                          + {item}
                        </li>
                      ))}
                    </ul>
                ...
                {compareResult.modified && compareResult.modified.length > 0 && (
                  <div className="bg-orange-50 border border-orange-200 rounded-lg p-3">
                    <h5 className="text-sm font-medium text-orange-900 mb-2">
                      Modifiés ({compareResult.modified.length})
                    </h5>
                    <ul className="space-y-1">
                      {compareResult.modified.map((item: string, idx: number) => (
                ...
                {compareResult.removed && compareResult.removed.length > 0 && (
                  <div className="bg-red-50 border border-red-200 rounded-lg p-3">
                    <h5 className="text-sm font-medium text-red-900 mb-2 flex items-center gap-2">
                      ...
                      Supprimés ({compareResult.removed.length})
                    </h5>
                    <ul className="space-y-1">
                      {compareResult.removed.map((item: string, idx: number) => (
