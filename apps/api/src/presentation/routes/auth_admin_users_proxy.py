...
async def proxy_mark_as_viewed(user_id: str, request: Request):
    """
    Proxy POST /api/auth/admin/users/{user_id}/mark-as-viewed
    to auth-microservice POST /api/admin/users/{user_id}/mark-as-viewed
    """
    target_url = f"{AUTH_SERVICE_URL}/api/admin/users/{user_id}/mark-as-viewed"
...
async def _proxy_request(target_url: str, request: Request):
    """
    Common proxy logic for all auth admin users endpoints
    """
    # Get query params
