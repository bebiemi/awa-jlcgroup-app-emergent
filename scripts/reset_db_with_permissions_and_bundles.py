#!/usr/bin/env python3
"""
Script de réinitialisation complète de la base de données IAM avec système de BUNDLES
- Nettoie toutes les permissions invalides
- Crée toutes les 160 permissions atomiques modernes
- Crée les BUNDLES de permissions (groupes cohérents)
...
# ==================== PERMISSIONS ATOMIQUES (160) ====================
# Permissions de bas niveau, gérées par les développeurs
...
ALL_ATOMIC_PERMISSIONS = [
    ...
    # MISSIONS - Workflow spécifique (nouveaux)
    ...
    # USERS - Actions spécifiques (nouveaux)
...
# Groupes cohérents de permissions atomiques
...
PERMISSION_BUNDLES = {
    ...
    "iam.full_access": {
        ...
        "permissions": [
            ...
            "iam.groups.edit",
            "iam.groups.delete",
            "iam.assign_permissions",
            "iam.revoke_permissions",
            "iam.audit",
        ]
    },
}
...
async def reset_database_with_bundles(delete_users=False):
    """Réinitialise la base de données IAM avec permissions atomiques + bundles"""
    client = AsyncIOMotorClient(MONGO_URL)
    db = client[DB_NAME]
    
    print("=" * 80)
    print(f"🔄 RÉINITIALISATION IAM AVEC SYSTÈME DE BUNDLES ({DB_NAME})")
    print("=" * 80)
    ...
    if bundles_to_insert:
        ...
        for bundle_code, bundle_data in PERMISSION_BUNDLES.items():
            print(f"      • {bundle_code}: {len(bundle_data['permissions'])} permissions")
    ...
    try:
        ...
        print(f"   ℹ️  Index déjà existants: {e}")
    
    # 5. Créer le profil SuperAdmin
    print("\n5️⃣ Création du profil SuperAdmin...")
    ...
    super_admin_profile = {
        ...
        "description": f"Profil avec tous les droits ({len(all_permission_codes)} permissions)",
    ...
    # 7. Supprimer les utilisateurs de test si demandé
    if delete_users:
        print("\n7️⃣ Suppression des utilisateurs de test...")
    ...
    print(f"   - Profil SuperAdmin avec accès complet")
    print("\n💡 Documentation:")
    print("   Voir /app/docs/BUNDLE_PERMISSIONS.md pour plus de détails")
    print("\n🚀 Prochaines étapes:")
    print("   1. Implémenter la résolution bundle → permissions atomiques")
    print("   2. Adapter les middlewares pour vérifier les bundles")
    print("   3. Créer les rôles métiers avec leurs bundles")
    print("=" * 80)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Réinitialiser la base IAM avec système de bundles")
    parser.add_argument(
        "--delete-users",
        action="store_true",
        help="Supprimer TOUS les utilisateurs (sauf admin)"
    )
    ...
    asyncio.run(reset_database_with_bundles(delete_users=args.delete_users))
